...
begin
    select out_code_id
      from i_model_custom_source_code('RP', 'AuthorizationLimitTheftFullLoss', "Theft Full Loss exceeds authorized limit", '', 1, 'TODAY', 'AUTHORIZE')
      into :var_code_id;
    
    /* Theft Full Loss Rules that DON'T create tasks */
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitTheftFullLoss', "Theft Full Loss exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 10001, 1000000, 'MGMTSR', 'UWSR', 'UW', null)
      into :var_auth_limit_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 1000001, 1250000, 'MGMTSR', 'UWSR', null, null)
      into :var_auth_limit_id;

    /* Theft Full Loss Rules that create tasks */
    select rule_category_id
      from rule_categories
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitTheftFullLoss_Notify', "Theft Full Loss exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 10001, 1000000, 'UWSR', 'MGMTSR', null, null)
      into :var_auth_limit_id;

    select activity_id
      from task_activity
     where activity_name = 'THEFTFULL_UW' rows 1
    ...
    select activity_id
      from task_activity
     where activity_name = 'THEFTFULL_UWSR' rows 1
      into :var_activity_id;
...
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ("Theft Full Loss exceeds 10,000", 5, "An application has been submitted with theft Full Loss coverage over 10,000 that needs your review.", 'THEFTFULL_UW', null, 1, 1, 'THEFT')^
 
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name) 
     values ("Theft Full Loss exceeds 1,000,000", 5,  "An application has been submitted with theft Full Loss coverage over 1,000,000 that needs your review.", 'THEFTFULL_UWSR', null, 1, 1, 'THEFT')^
...
execute procedure temp_create_view_activity_right('THEFTFULL_UW', 'AG')^
execute procedure temp_create_view_activity_right('THEFTFULL_UW', 'UW')^
execute procedure temp_create_view_activity_right('THEFTFULL_UW', 'UWSR')^
execute procedure temp_create_view_activity_right('THEFTFULL_UW', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('THEFTFULL_UW', 'UW')^
execute procedure temp_create_edit_activity_right('THEFTFULL_UW', 'UWSR')^
execute procedure temp_create_edit_activity_right('THEFTFULL_UW', 'MGMTSR')^

execute procedure temp_create_view_activity_right('THEFTFULL_UWSR', 'AG')^
execute procedure temp_create_view_activity_right('THEFTFULL_UWSR', 'UW')^
execute procedure temp_create_view_activity_right('THEFTFULL_UWSR', 'UWSR')^
execute procedure temp_create_view_activity_right('THEFTFULL_UWSR', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('THEFTFULL_UWSR', 'UWSR')^
execute procedure temp_create_edit_activity_right('THEFTFULL_UWSR', 'MGMTSR')^
...
insert into applied_scripts (name, description, script_date)
     values ('201020ML08', 'RT-161 - auth level for theft full loss', '2020-Sept-20');