...
begin
    select out_code_id
      from i_model_custom_source_code('AuthorizationLimitFire' || :in_line, "Fire exceeds authorized limit", '', 1, 'TODAY', 'AUTHORIZE')
      into :var_code_id;
    
    /* Fire Rules that DON'T create tasks */
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, "Fire exceeds authorized limit", :var_code_id, 1, 'TODAY')
      into :var_detail_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 250001, 5000000, 'MGMTSR', 'UWSR', 'UWMGR', NULL)
      into :var_auth_limit_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 5000001, 10000000, 'MGMTSR', 'UWMGR', NULL,NULL)
      into :var_auth_limit_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 10000001, 20000000, 'MGMTSR', NULL, NULL, NULL)
      into :var_auth_limit_id;

    /* Fire Rules that create tasks */
    select rule_category_id
      from rule_categories
    ...
    select activity_id
      from task_activity
     where activity_name = 'FIRE_UWSR' rows 1
    ...
    select activity_id
      from task_activity
     where activity_name = 'FIRE_UWMGR' rows 1
      into :var_activity_id;
    ...
    select activity_id
      from task_activity
     where activity_name = 'FIRE_MGMTSR' rows 1
      into :var_activity_id;
...
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ("Fire exceeds Underwriter limit", 5, "An application has been submitted with Fire that requires Senior Underwriter review.", 'FIRE_UWSR', null, 1, 1, 'FIRE')^

insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ("Fire exceeds Senior Underwriter limit", 5, "An application has been submitted with Fire that requires Underwriting Manager review.", 'FIRE_UWMGR', null, 1, 1, 'FIRE')^

insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ("Fire exceeds Underwriting Manager limit", 5, "An application has been submitted with Fire that requires Senior Management review.", 'FIRE_MGMTSR', null, 1, 1, 'FIRE')^
...
execute procedure temp_create_view_activity_right('FIRE_UWSR', 'AG')^
execute procedure temp_create_view_activity_right('FIRE_UWSR', 'UW')^
execute procedure temp_create_view_activity_right('FIRE_UWSR', 'UWSR')^
execute procedure temp_create_view_activity_right('FIRE_UWSR', 'UWMGR')^
execute procedure temp_create_view_activity_right('FIRE_UWSR', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('FIRE_UWSR', 'UWSR')^
execute procedure temp_create_edit_activity_right('FIRE_UWSR', 'UWMGR')^
execute procedure temp_create_edit_activity_right('FIRE_UWSR', 'MGMTSR')^

execute procedure temp_create_view_activity_right('FIRE_UWMGR', 'AG')^
execute procedure temp_create_view_activity_right('FIRE_UWMGR', 'UW')^
execute procedure temp_create_view_activity_right('FIRE_UWMGR', 'UWSR')^
execute procedure temp_create_view_activity_right('FIRE_UWMGR', 'UWMGR')^
execute procedure temp_create_view_activity_right('FIRE_UWMGR', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('FIRE_UWMGR', 'UWMGR')^
execute procedure temp_create_edit_activity_right('FIRE_UWMGR', 'MGMTSR')^

execute procedure temp_create_view_activity_right('FIRE_MGMTSR', 'AG')^
execute procedure temp_create_view_activity_right('FIRE_MGMTSR', 'UW')^
execute procedure temp_create_view_activity_right('FIRE_MGMTSR', 'UWSR')^
execute procedure temp_create_view_activity_right('FIRE_MGMTSR', 'UWMGR')^
execute procedure temp_create_view_activity_right('FIRE_MGMTSR', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('FIRE_MGMTSR', 'MGMTSR')^
...
insert into applied_scripts(name, description, script_date)
     values ("201123ML05", "RT-178 - inserted missing rules", "2020-NOV-23");