...
begin
    select out_code_id
      from i_model_custom_source_code('RP', 'AuthorizationLimitTheftFirstLoss', "Theft First Loss exceeds authorized limit", '', 1, 'TODAY', 'AUTHORIZE')
      into :var_code_id;
    
    /* Theft First Loss Rules that DON'T create tasks */
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitTheftFirstLoss', "Theft First Loss exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 10001, 100000, 'MGMTSR', 'UWSR', 'UW', null)
      into :var_auth_limit_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 100001, 250000, 'MGMTSR', 'UWSR', null, null)
      into :var_auth_limit_id;

    /* Theft First Loss Rules that create tasks */
    select rule_category_id
      from rule_categories
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitTheftFirstLoss_Notify', "Theft First Loss exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 10001, 100000, 'UWSR', 'MGMTSR', null, null)
      into :var_auth_limit_id;

    select activity_id
      from task_activity
     where activity_name = 'THEFTFIRST_UW' rows 1
    ...
    select activity_id
      from task_activity
     where activity_name = 'THEFTFIRST_UWSR' rows 1
      into :var_activity_id;
...
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ("Theft First Loss exceeds 10,000", 5, "An application has been submitted with Theft First Loss coverage over 10,000 that needs your review.", 'THEFTFIRST_UW', null, 1, 1, 'THEFT')^
 
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name) 
     values ("Theft First Loss exceeds 100,000", 5,  "An application has been submitted with Theft First Loss coverage over 100,000 that needs your review.", 'THEFTFIRST_UWSR', null, 1, 1, 'THEFT')^
...
execute procedure temp_create_view_activity_right('THEFTFIRST_UW', 'AG')^
execute procedure temp_create_view_activity_right('THEFTFIRST_UW', 'UW')^
execute procedure temp_create_view_activity_right('THEFTFIRST_UW', 'UWSR')^
execute procedure temp_create_view_activity_right('THEFTFIRST_UW', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('THEFTFIRST_UW', 'UW')^
execute procedure temp_create_edit_activity_right('THEFTFIRST_UW', 'UWSR')^
execute procedure temp_create_edit_activity_right('THEFTFIRST_UW', 'MGMTSR')^

execute procedure temp_create_view_activity_right('THEFTFIRST_UWSR', 'AG')^
execute procedure temp_create_view_activity_right('THEFTFIRST_UWSR', 'UW')^
execute procedure temp_create_view_activity_right('THEFTFIRST_UWSR', 'UWSR')^
execute procedure temp_create_view_activity_right('THEFTFIRST_UWSR', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('THEFTFIRST_UWSR', 'UWSR')^
execute procedure temp_create_edit_activity_right('THEFTFIRST_UWSR', 'MGMTSR')^
...
insert into applied_scripts (name, description, script_date)
     values ('201020ML09', 'RT-161 - auth level for theft first loss', '2020-Sept-20');