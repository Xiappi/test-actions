...
begin
    select out_code_id
      from i_model_custom_source_code('RP', 'AuthorizationLimitEmployersLiability', "Employer's Liability exceeds authorized limit", '', 1, 'TODAY', 'AUTHORIZE')
      into :var_code_id;
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitEmployersLiability', "Employer's Liability exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 10000, 2500000, 'UW', 'UWSR', 'MGMTSR', null)
      into :var_auth_limit_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 2500001, 5000000, 'MGMTSR', null, null, null)
      into :var_auth_limit_id;
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitEmployersLiability_Notify', "Employer's Liability exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;
    ...
    select activity_id
      from task_activity
     where activity_name = 'EMPLOYERLIABILITY_UW' rows 1
    ...
    select activity_id
      from task_activity
     where activity_name = 'EMPLOYERLIABILITY_MGMTSR' rows 1
      into :var_activity_id;
...
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ("Employer's Liability exceeds 10,000", 5, "An application has been submitted with employer's liability coverage over 10,000 that needs your review.", 'EMPLOYERLIABILITY_UW', null, 1, 1, 'EMPLIAB')^
 
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name) 
     values ("Employer's Liability exceeds 2,500,000", 5,  "An application has been submitted with employer's liability coverage over 2,500,000 that needs your review.", 'EMPLOYERLIABILITY_MGMTSR', null, 1, 1, 'EMPLIAB')^
...
execute procedure temp_create_view_activity_right('EMPLOYERLIABILITY_UW', 'AG')^
execute procedure temp_create_view_activity_right('EMPLOYERLIABILITY_UW', 'UW')^
execute procedure temp_create_view_activity_right('EMPLOYERLIABILITY_UW', 'UWSR')^
execute procedure temp_create_view_activity_right('EMPLOYERLIABILITY_UW', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('EMPLOYERLIABILITY_UW', 'UW')^
execute procedure temp_create_edit_activity_right('EMPLOYERLIABILITY_UW', 'UWSR')^
execute procedure temp_create_edit_activity_right('EMPLOYERLIABILITY_UW', 'MGMTSR')^

execute procedure temp_create_view_activity_right('EMPLOYERLIABILITY_MGMTSR', 'AG')^
execute procedure temp_create_view_activity_right('EMPLOYERLIABILITY_MGMTSR', 'UW')^
execute procedure temp_create_view_activity_right('EMPLOYERLIABILITY_MGMTSR', 'UWSR')^
execute procedure temp_create_view_activity_right('EMPLOYERLIABILITY_MGMTSR', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('EMPLOYERLIABILITY_MGMTSR', 'MGMTSR')^
...
insert into applied_scripts (name, description, script_date)
     values ('201018ML01', 'RT-161 - auth level for employers liab coverage', '2020-Sept-18');