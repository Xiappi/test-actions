...
begin
    ...
    select out_id
      from s_model_custom_source_code_types
     where out_name = 'AUTHORIZE'
    ...
    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 1, 1000, 'UW', 'UWSR', 'UWMGR', 'MGMTSR')
      into :var_auth_limit_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 1001, 2500, 'UWMGR', 'MGMTSR', null, null)
      into :var_auth_limit_id;
...
begin
    ...
    select activity_id
      from task_activity
     where activity_name = 'PEDAL_CYCLE_UW' rows 1
    ...
    select activity_id
      from task_activity
     where activity_name = 'PEDAL_CYCLE_UWMGR' rows 1
      into :var_activity_id;
...
/* Create Pedal Cycle task activities */
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ('Pedal Cycles exceeds Broker limit', 5, 'An application has been submitted with pedal cycle coverage that requires Underwriter review.', 'PEDAL_CYCLE_UW', null, 1, 1, 'PEDAL')^
 
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name) 
     values ('Pedal Cycles exceeds Underwriter limit', 5,  'An application has been submitted with pedal cycle coverage that requires Underwriting Manager review.', 'PEDAL_CYCLE_UWMGR', null, 1, 1, 'PEDAL')^

/* Add rights to the new task activity */
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UW', 'AG', 'VIEW')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UW', 'UW', 'VIEW')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UW', 'UWSR', 'VIEW')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UW', 'MGMTSR', 'VIEW')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UW', 'UWMGR', 'VIEW')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UW', 'UW', 'EDIT')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UW', 'UWSR', 'EDIT')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UW', 'MGMTSR', 'EDIT')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UW', 'UWMGR', 'EDIT')^

execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UWMGR', 'AG', 'VIEW')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UWMGR', 'UW', 'VIEW')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UWMGR', 'UWSR', 'VIEW')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UWMGR', 'MGMTSR', 'VIEW')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UWMGR', 'UWMGR', 'VIEW')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UWMGR', 'UWSR', 'EDIT')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UWMGR', 'MGMTSR', 'EDIT')^
execute procedure temp_configure_task_acitivity_rights('PEDAL_CYCLE_UWMGR', 'UWMGR', 'EDIT')^


execute procedure temp_create_rule_without_notify('AuthorizationLimitPedalCycles', 'Pedal Cycles exceeds authorized limit')^
execute procedure temp_create_rule_with_notify('AuthorizationLimitPedalCycles', 'Pedal Cycles exceeds authorized limit')^
...
insert into applied_scripts (name, description, script_date)
     values ('210708DC01', 'MHM-250 add UW rule for pedal cycle auth limit', '2021-Jul-08');