...
begin
    ...
    insert into custom_source_code (code_id, name, description, source_code, source_code_type_id)
         values (:var_code_id, 'AddCCTVDiscountAutoIncludeItem', 'Auto-include item cctv discount.', '', :var_source_code_type_id);
 
    select rule_category_id 
      from rule_categories 
     where rule_code = 'AUTOINCLUDE_ITEM_RULES' 
    ...
    insert into rule_details(rule_detail_id, code_id, fail_message, rule_active, effective_book_id, sequence)
        values (:var_detail_id, :var_code_id, 'Auto-include item cctv discount.', 1, :var_book_id, 0);
...
insert into applied_scripts(name, description, script_date)
     values ("210823AO02", "MHM-729 - HX CCTV auto-include item rule", "2021-AUG-23");
