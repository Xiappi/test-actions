...
begin
    select out_code_id
      from i_model_custom_source_code('RP', 'AuthorizationLimitPublicLiability', "Public Liability exceeds authorized limit", '', 1, 'TODAY', 'AUTHORIZE')
      into :var_code_id;
    
    /* Public Liability Rules that DON'T create tasks */
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitPublicLiability', "Public Liability exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 10000, 2000000, 'UW', 'UWSR', 'MGMTSR', null)
    ...
    /* Public Liability Rules that create tasks */
    select rule_category_id
      from rule_categories
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitPublicLiability_Notify', "Public Liability exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;
    ...
    select activity_id
      from task_activity
     where activity_name = 'PUBLICLIABILITY_UW' rows 1
    ...
    select activity_id
      from task_activity
     where activity_name = 'PUBLICLIABILITY_MGMTSR' rows 1
      into :var_activity_id;
...
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ("Public Liability exceeds 10,000", 5, "An application has been submitted with public liability coverage over 10,000 that needs your review.", 'PUBLICLIABILITY_UW', null, 1, 1, 'PUBLIAB')^
 
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name) 
     values ("Public Liability exceeds 2,000,000", 5,  "An application has been submitted with public liability coverage over 2,000,000 that needs your review.", 'PUBLICLIABILITY_MGMTSR', null, 1, 1, 'PUBLIAB')^
...
execute procedure temp_create_view_activity_right('PUBLICLIABILITY_UW', 'AG')^
execute procedure temp_create_view_activity_right('PUBLICLIABILITY_UW', 'UW')^
execute procedure temp_create_view_activity_right('PUBLICLIABILITY_UW', 'UWSR')^
execute procedure temp_create_view_activity_right('PUBLICLIABILITY_UW', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('PUBLICLIABILITY_UW', 'UW')^
execute procedure temp_create_edit_activity_right('PUBLICLIABILITY_UW', 'UWSR')^
execute procedure temp_create_edit_activity_right('PUBLICLIABILITY_UW', 'MGMTSR')^

execute procedure temp_create_view_activity_right('PUBLICLIABILITY_MGMTSR', 'AG')^
execute procedure temp_create_view_activity_right('PUBLICLIABILITY_MGMTSR', 'UW')^
execute procedure temp_create_view_activity_right('PUBLICLIABILITY_MGMTSR', 'UWSR')^
execute procedure temp_create_view_activity_right('PUBLICLIABILITY_MGMTSR', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('PUBLICLIABILITY_MGMTSR', 'MGMTSR')^
...
insert into applied_scripts (name, description, script_date)
     values ('201020ML01', 'RT-161 - auth level for public liab coverage', '2020-Sept-20');