...
begin
    select out_code_id
      from i_model_custom_source_code('RP', 'AuthorizationLimitTerrorism', "Terrorism exceeds authorized limit", '', 1, 'TODAY', 'AUTHORIZE')
      into :var_code_id;
    
    /* Terrorism Rules that DON'T create tasks */
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitTerrorism', "Terrorism exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 10001, 500000, 'MGMTSR', null, null, null)
      into :var_auth_limit_id;

    /* Terrorism Rules that create tasks */
    select rule_category_id
      from rule_categories
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitTerrorism_Notify', "Terrorism exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;
    ...
    select activity_id
      from task_activity
     where activity_name = 'TERRORISM_MGMTSR' rows 1
...
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ("Terrorism exceeds 10,000", 5, "An application has been submitted with Terrorism coverage over 10,000 that needs your review.", 'TERRORISM_MGMTSR', null, 1, 1, 'TERRORISM')^
...
execute procedure temp_create_view_activity_right('TERRORISM_MGMTSR', 'AG')^
execute procedure temp_create_view_activity_right('TERRORISM_MGMTSR', 'UW')^
execute procedure temp_create_view_activity_right('TERRORISM_MGMTSR', 'UWSR')^
execute procedure temp_create_view_activity_right('TERRORISM_MGMTSR', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('TERRORISM_MGMTSR', 'MGMTSR')^
...
insert into applied_scripts (name, description, script_date)
     values ('201020ML10', 'RT-161 - auth level for terrorism', '2020-Sept-20');