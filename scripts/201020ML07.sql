...
begin
    select out_code_id
      from i_model_custom_source_code('RP', 'AuthorizationLimitMachinery', "Machinery Breakdown - Electronic Equipment exceeds authorized limit", '', 1, 'TODAY', 'AUTHORIZE')
      into :var_code_id;
    
    /* Machinery Breakdown - Electronic Equipment Rules that DON'T create tasks */
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitMachinery', "Machinery Breakdown - Electronic Equipment exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 10001, 250000, 'MGMTSR', 'UWSR', 'UW', null)
      into :var_auth_limit_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 250001, 5000000, 'MGMTSR', 'UWSR', null, null)
      into :var_auth_limit_id;

    /* Machinery Breakdown - Electronic Equipment Rules that create tasks */
    select rule_category_id
      from rule_categories
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, 'AuthorizationLimitMachinery_Notify', "Machinery Breakdown - Electronic Equipment exceeds authorized limit", :var_code_id, 1, 'TODAY', 1, 1, 1, 0)
      into :var_detail_id;

    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 10001, 250000, 'UWSR', 'MGMTSR', null, null)
      into :var_auth_limit_id;

    select activity_id
      from task_activity
     where activity_name = 'MACHINERY_UW' rows 1
    ...
    select activity_id
      from task_activity
     where activity_name = 'MACHINERY_UWSR' rows 1
      into :var_activity_id;
...
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ("Machinery Breakdown - Electronic Equipment exceeds 10,000", 5, "An application has been submitted with machinery breakdown - electronic equipment coverage over 10,000 that needs your review.", 'MACHINERY_UW', null, 1, 1, 'MACHINERY')^
 
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name) 
     values ("Machinery Breakdown - Electronic Equipment exceeds 250,000", 5,  "An application has been submitted with machinery breakdown - electronic equipment coverage over 250,000 that needs your review.", 'MACHINERY_UWSR', null, 1, 1, 'MACHINERY')^
...
execute procedure temp_create_view_activity_right('MACHINERY_UW', 'AG')^
execute procedure temp_create_view_activity_right('MACHINERY_UW', 'UW')^
execute procedure temp_create_view_activity_right('MACHINERY_UW', 'UWSR')^
execute procedure temp_create_view_activity_right('MACHINERY_UW', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('MACHINERY_UW', 'UW')^
execute procedure temp_create_edit_activity_right('MACHINERY_UW', 'UWSR')^
execute procedure temp_create_edit_activity_right('MACHINERY_UW', 'MGMTSR')^

execute procedure temp_create_view_activity_right('MACHINERY_UWSR', 'AG')^
execute procedure temp_create_view_activity_right('MACHINERY_UWSR', 'UW')^
execute procedure temp_create_view_activity_right('MACHINERY_UWSR', 'UWSR')^
execute procedure temp_create_view_activity_right('MACHINERY_UWSR', 'MGMTSR')^
execute procedure temp_create_edit_activity_right('MACHINERY_UWSR', 'UWSR')^
execute procedure temp_create_edit_activity_right('MACHINERY_UWSR', 'MGMTSR')^
...
insert into applied_scripts (name, description, script_date)
     values ('201020ML07', 'RT-161 - auth level for machinery', '2020-Sept-20');