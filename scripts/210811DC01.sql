...
begin
    ...
    select out_auth_limit_id
      from tmp_create_limits(:var_detail_id, 501, 1000, 'UW', 'UWSR', 'UWMGR', 'MGMTSR')
...
begin
    ...
    select activity_id
      from task_activity
     where activity_name = 'MONEY_HX_UW' rows 1
    ...
    select activity_id
      from task_activity
     where activity_name = 'MONEY_HX_UWMGR' rows 1
      into :var_activity_id;
...
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ('Money exceeds Broker limit', 5, 'An application has been submitted with money coverage that requires Underwriter review.', 'MONEY_HX_UW', null, 1, 1, 'MONEY')^
 
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name) 
     values ('Money exceeds Senior Underwriter limit', 5,  'An application has been submitted with money coverage that requires Underwriting Manager review.', 'MONEY_HX_UWMGR', null, 1, 1, 'MONEY')^

/* Add rights to the new task activity */
execute procedure temp_configure_task_activity_rights('MONEY_HX_UW', 'AG', 'VIEW')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UW', 'UW', 'VIEW')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UW', 'UWSR', 'VIEW')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UW', 'MGMTSR', 'VIEW')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UW', 'UWMGR', 'VIEW')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UW', 'UW', 'EDIT')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UW', 'UWSR', 'EDIT')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UW', 'MGMTSR', 'EDIT')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UW', 'UWMGR', 'EDIT')^

execute procedure temp_configure_task_activity_rights('MONEY_HX_UWMGR', 'AG', 'VIEW')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UWMGR', 'UW', 'VIEW')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UWMGR', 'UWSR', 'VIEW')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UWMGR', 'MGMTSR', 'VIEW')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UWMGR', 'UWMGR', 'VIEW')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UWMGR', 'MGMTSR', 'EDIT')^
execute procedure temp_configure_task_activity_rights('MONEY_HX_UWMGR', 'UWMGR', 'EDIT')^


execute procedure temp_create_rule_without_notify('AuthorizationLimitMoneyHX', 'Money exceeds authorized limit')^
execute procedure temp_create_rule_with_notify('AuthorizationLimitMoneyHX', 'Money exceeds authorized limit')^
...
insert into applied_scripts (name, description, script_date)
     values ('210811DC01', 'MHM-105 add rule for money and auth limit', '2021-Aug-11');