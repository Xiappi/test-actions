...
begin
    select code_id
     ...
     where name = 'AuthorizationLimitBuildingsRent'
      into :var_code_id;
    
    /* Buildings/Rent Rules that DON'T create tasks */
    ...
    select out_detail_id
      from i_model_rule_details(:var_master_id, "Buildings/Rent exceeds authorized limit", :var_code_id, 1, 'TODAY')
      into :var_detail_id;
    ...
    /* Buildings/Rent Rules that create tasks */
    select rule_category_id
      from rule_categories
    ...
    select activity_id
      from task_activity
     where activity_name = 'BUILDINGSRENT_UW' rows 1
...
insert into task_activity (task_title, default_number_of_days_due, task_message, activity_name, web_link, notify_agent, notify_creator, category_name)
     values ("Buildings/Rent exceeds Broker limit", 5, "An application has been submitted with Buildings/Rent that requires Underwriter review.", 'BUILDINGSRENT_UW', null, 1, 1, 'BUILDRENT')^

commit work^

execute procedure i_model_custom_source_code('AuthorizationLimitBuildingsRent', "Buildings/Rent exceeds authorized limit", '', 1, 'TODAY', 'AUTHORIZE')^
...
execute procedure temp_create_view_activity_right('BUILDINGSRENT_UW', 'AG')^
execute procedure temp_create_view_activity_right('BUILDINGSRENT_UW', 'UW')^
execute procedure temp_create_view_activity_right('BUILDINGSRENT_UW', 'UWSR')^
execute procedure temp_create_view_activity_right('BUILDINGSRENT_UW', 'UWMGR')^
execute procedure temp_create_view_activity_right('BUILDINGSRENT_UW', 'MGMTSR')^

execute procedure temp_create_edit_activity_right('BUILDINGSRENT_UW', 'UW')^
execute procedure temp_create_edit_activity_right('BUILDINGSRENT_UW', 'UWSR')^
execute procedure temp_create_edit_activity_right('BUILDINGSRENT_UW', 'UWMGR')^
execute procedure temp_create_edit_activity_right('BUILDINGSRENT_UW', 'MGMTSR')^
...
insert into applied_scripts(name, description, script_date)
     values ("201123ML04", "RT-178 - inserted missing rules", "2020-NOV-23");